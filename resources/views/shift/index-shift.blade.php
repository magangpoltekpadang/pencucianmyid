...
@section('content')
    <div x-data="shiftData()" x-init="init()" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-500">Active Shift</p>
                        <p class="mt-1 text-3xl font-semibold text-gray-900"
                            x-text="shifts.filter(s => s.is_active).length">
                        </p>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-500">Inactive Shift</p>
                        <p class="mt-1 text-3xl font-semibold text-gray-900"
                            x-text="shifts.filter(s => !s.is_active).length">
                        </p>
                    </div>
        ...
        <div class="bg-white rounded-lg shadow p-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" id="search" x-model="search" @change="fetchShifts()" @input="fetchShifts()"
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <template x-for="shift in shifts" :key="shift.shift_id">
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="shift.shift_name">
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="shift.outlet_id">
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="shift.start_time">
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" x-text="shift.end_time">
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        x-bind:class="shift.is_active ? 'bg-green-100 text-green-800' :
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a :href="`/shifts/${shift.shift_id}/edit`"
                                        class="text-blue-600 hover:text-blue-900 mr-3">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <button @click="confirmDelete(shift.shift_id)" class="text-red-600 hover:text-red-900">
                        ...
                        <tr x-show="shifts.length === 0">
                            <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">No Shift Found
                            </td>
                        </tr>
                    </tbody>
                </table>
    ...
    @push('scripts')
        <script src="{{ asset('js\shifts\shift-script.js') }}"></script>
    @endpush
@endsection
