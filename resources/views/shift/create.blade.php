...
@section('content')
    <div class="max-w-xl mx-auto px-4">
        <form action="/shifts" method="POST" class="max-w-xl mx-auto p-6 bg-white rounded shadow">
            @csrf
            ...
            <div class="mb-4">
                ...
                <input type="text" name="shift_name" id="shift_name" autocomplete="shift_name"
                    ...
                    placeholder="shift name" value="{{ old('shift_name') }}" required>
                @error('shift_name')
            ...
            <!-- Start Time -->
             <div class="mb-4">
                ...
                <input type="time" step="1" name="start_time" id="start_time"
                    class="block w-full h-11 px-5 py-2.5 text-base font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-400 focus:outline-none"
                    placeholder="start_time" value="{{ old('start_time') }}" required>
                @error('start_time')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
             ...
             <div class="mb-4">
                ...
                <input type="time" step="1" name="end_time" id="end_time"
                    class="block w-full h-11 px-5 py-2.5 text-base font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-400 focus:outline-none"
                    placeholder="end_time" value="{{ old('end_time') }}" required>
                @error('end_time')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
