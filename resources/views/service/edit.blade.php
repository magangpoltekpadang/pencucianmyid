...
@section('content')
    <div class="max-w-xl mx-auto px-4">
        <form action="{{ url('services/' . $services->service_id) }}" method="POST" class="max-w-xl mx-auto p-6 bg-white rounded shadow">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <h1 class="text-xl font-bold text-gray-900 mb-4">Edit Data Service</h1>
                ...
                <input type="text" name="service_name" id="service_name" autocomplete="service_name"
                    ...
                    placeholder="service_name" value="{{ old('service_name', $services->service_name) }}" required>
                @error('service_name')
        ...
        <div class="mb-4">
            ...
            <select id="service_type_id" name="service_type_id" required autocomplete="service_type_id"
                class="mt-1 block w-full rounded-md border border-gray-300 p-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm">
                @foreach ($service_types as $service_type)
                    <option value="{{ $service_type->service_type_id }}" {{ $services->outlet_id == $service_type->service_type_id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="price" id="price" autocomplete="price"
                    class="block w-full h-11 px-5 py-2.5 text-base font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-400 focus:outline-none"
                    placeholder="price" value="{{ old('price', $services->price) }}" required>
                @error('price')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="estimated_duration" id="estimated_duration" autocomplete="estimated_duration"
                    class="block w-full h-11 px-5 py-2.5 text-base font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-400 focus:outline-none"
                    placeholder="estimated_duration"
                    value="{{ old('estimated_duration', $services->estimated_duration) }}" required>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="4" autocomplete="description"
                    ...
                    placeholder="Write description..." required>{{ old('description', $services->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mb-4">
            ...
            <select id="outlet_id" name="outlet_id" required autocomplete="outlet_id"
                class="mt-1 block w-full rounded-md border border-gray-300 p-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm">
                @foreach ($outlets as $outlet)
                    <option value="{{ $outlet->outlet_id }}" {{ $services->outlet_id == $outlet->outlet_id ? 'selected' : '' }}>
                        {{ $outlet->outlet_name }}
                    </option>
                @endforeach
            </select>
            ...
            <div class="mb-4">
                ...
                <select name="is_active" id="is_active" autocomplete="is_active"
                    ...
                    <option value="1" {{ $services->is_active == '1' ? 'selected' : '' }}>Active</option>
                    <option value="0" {{ $services->is_active == '0' ? 'selected' : '' }}>Inactive</option>
                </select>
                @error('is_active')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
